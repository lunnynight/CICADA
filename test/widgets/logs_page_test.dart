import 'package:flutter/material.dart';
import 'package:flutter_test/flutter_test.dart';
import '../../lib/pages/logs_page.dart';

void main() {
  group('LogsPage', () {
    Widget buildPage() => const MaterialApp(home: Scaffold(body: LogsPage()));

    Future<void> pumpAndWait(WidgetTester tester, Widget widget) async {
      await tester.runAsync(() async {
        await tester.pumpWidget(widget);
        await Future.delayed(const Duration(milliseconds: 500));
      });
      await tester.pump();
      await tester.pump();
    }

    testWidgets('renders without crashing', (tester) async {
      await pumpAndWait(tester, buildPage());
      expect(find.byType(LogsPage), findsOneWidget);
    });

    testWidgets('shows loading or content state', (tester) async {
      await pumpAndWait(tester, buildPage());
      expect(find.byType(LogsPage), findsOneWidget);
    });

    testWidgets('completes async load without error', (tester) async {
      await pumpAndWait(tester, buildPage());
      expect(find.byType(Scaffold), findsWidgets);
    });

    testWidgets('disposes stream subscription without error', (tester) async {
      await pumpAndWait(tester, buildPage());
      await tester.runAsync(() async {
        await tester.pumpWidget(const MaterialApp(home: Scaffold()));
        await Future.delayed(const Duration(milliseconds: 100));
      });
      await tester.pump();
    });
  });
}
